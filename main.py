...
# End of today (UTC)
# end_time = start_time + timedelta(hours=2, minutes=59, seconds=59)
...
def find_rate_cz(html):
    ...
    if table:
        ...
        usd_mask = df['Currency'].str.contains('dollar', case=False, na=False) | \
                    df['Code'].str.contains('USD', case=False, na=False) | \
                    df['Country'].str.contains('USA', case=False, na=False)
        
        eur_row = df[eur_mask]
        usd_row = df[usd_mask]
        if not eur_row.empty and not usd_row.empty:
            eur_res = float(eur_row['Rate'].iloc[0])
            eur_rate = 1/eur_res
            usd_res = float(usd_row['Rate'].iloc[0])
            usd_rate = 1/usd_res
            return usd_rate, eur_rate
        elif not eur_row.empty:
            print("No EUR row found")
        else:
            print("No USD row found")
...
if __name__ == "__main__":
    with open('output.txt', 'w', encoding='utf-8') as f:
        ...
        usd_rate, eur_rate = get_exchange_rate(CZ_url, 'CZ')
        # mx_rate = get_exchange_rate(MX_url, 'MX')
        # print("pt_rate: ", pt_rate)
        print("usd_rate: ", usd_rate)
        print("eur_rate: ", eur_rate)
        # print("mx_rate: ", mx_rate)
        # print(update_erp_data(PT_headers, PT_ERP_API_BASE, dollar_key, pt_rate))
        print(update_erp_data(CZ_headers, CZ_ERP_API_BASE, euro_key, eur_rate))
        print(update_erp_data(CZ_headers, CZ_ERP_API_BASE, dollar_key, usd_rate))
